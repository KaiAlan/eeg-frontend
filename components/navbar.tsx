      ...
      <div className="max-w-[1512px] w-full flex justify-between items-center gap-6 py-4 pt-6 mx-auto text-lg text-typeface-2 px-8">
        <div className="flex justify-start items-center gap-10">
          ...
          {NavConfig.mainNav.map((item, index) => (
            <Link
              ...
              href={item.href}
              className={
                cn("flex justify-start items-center gap-0.5 font-medium uppercase text-black")
              }
            >
              {item.title}
              <img src="/icons/expand-more.svg" alt="Logo" className="w-8 font-bold" />
            </Link>
          ))}
        </div>
        <div className="flex justify-end items-center gap-6 h-12 w-full">
          <div className="w-full h-full flex justify-end items-center">
            ...
            />
            <Button variant='menu' className="bg-white h-full rounded-3xl rounded-l-none shadow-md hover:border-white hover:bg-primary border-muted border-l-0">
              <img src="/icons/search.svg" alt="Logo" className="w-8 z-[70] font-bold text-white" />
            </Button>
          </div>
        <Link
            href="/auth/login"
          ...
          >
            <img src="/icons/person-outline.svg" alt="Logo" className="w-8 z-[70] font-bold" />
          </Link>
        <Link
            href="/"
            className={
          ...
          >
            <img src="/icons/work-outline.svg" alt="Logo" className="w-8 z-[70] font-bold" />
          </Link>
        </div>
      </div>
      <div className="w-full h-12 bg-primary">
            <div className="max-w-[1512px] w-full h-full flex justify-start items-center gap-4 px-8 mx-auto overflow-hidden">
              ...
              {Category.map((catalog, index) => {
                ...
                return (
                  <Button key={index} variant='menu' className="px-2" onClick={() => router.push(`/category/${catalog.name}`)}>{catalog.name}</Button>
                )
              })}
            </div>
      </div>
