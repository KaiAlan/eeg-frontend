...
const SearchResults = ({ searchKey }: { searchKey: string }) => {
  ...
  useEffect(() => {
    ...
    if (filterSellers) {
      ...
      // Set filtered data only if it is different
      if (
        filteredProducts.length !== filteredData.length ||
        !filteredProducts.every(
          (value, index) => value.productId === filteredData[index]?.productId
        )
      ) {
    ...
    <div className="flex mt-8 relative pt-10">
      ...
      <div className="max-w-[1176px] w-full flex flex-col justify-start px-4 mx-auto desktop:max-w-full desktop:mx-0 desktop:pl-12">
        <span className="text-muted-foreground text-sm">
          Showiing results for: <span className="text-black">{searchKey}</span>
        </span>
        <div className="w-full grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6 pt-4 relative">
